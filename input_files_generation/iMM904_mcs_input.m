...
%%  base network
load yeastMM904.mat yeastMM904 del_exchanges exchange_ind noKO3 atpm glucose mue
st= yeastMM904.stoichMat;
irr= yeastMM904.reacMin >= 0;
reac_off_ref= del_exchanges;
separate_reac= unique([exchange_ind, noKO3, atpm, glucose, mue]);
separate_reac(end+1)= 0; % placeholder for index of the export reaction

%% MCS parameters
glucose_uptake_limit= 10;
...
sub_rat= cell(1, q);
val_red= NaN(1, q);
load yeast904_KO3_10_dx.mat rd_rat irrev_rd_rat sub_rat fvalb fvaub val_red proper_mcs
...
for i= idx
  ...
  reac_off= setdiff(reac_off_ref, production);
...
%     gluc_up(i)= -res.x(glucr);
  ...
  inh{i}(3, glucr)= 1; % lower limit on glucose uptake to exclude 0 from the target system
  inh{i}(4, atpmr)= -1;
  ub{i}= [glucose_uptake_limit; 0; -0.1; -min_atpm];
...
%%
save iMM904_mcs_input.mat rd_rat irrev_rd_rat sub_rat flux_lb flux_ub gluc_up max_prod cuts kn idx inh ub des db
