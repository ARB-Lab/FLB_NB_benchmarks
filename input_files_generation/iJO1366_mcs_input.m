...
%% base network
load orth1366D.mat orth1366Bs noKO exchange_ind atpm glucose mue o2in reac_off del_exchanges
...
sub_rat= cell(1, q);
val_red= NaN(1, q);
load orthD_10_dx.mat rd_rat irrev_rd_rat sub_rat fvalb fvaub val_red proper_mcs
...
for i= idx
  ...
  if isempty(sub_rat{i})
    ...
    lhs= [zeros(coli_full.nums, 1); -Inf(length(dbf), 1)];
    rhs= [zeros(coli_full.nums, 1); dbf];
...
%     gluc_up(i)= -res.x(glucr);
  ...
  cuts{i}(any(sub(:, noKO), 2))= false;
  kn{i}= null_rat_efmtool(rd);
end

%%
save iJO1366_mcs_input.mat rd_rat irrev_rd_rat sub_rat flux_lb flux_ub gluc_up max_prod cuts kn idx inh ub des db
